...
int main(int argc, char** argv) {
    ...
    bool JouerContreOrdi = false;
    int player = 1;
    
    // Initialisation de la graine aléatoire pour le choix de l'ordinateur
    std::srand(std::time(0));

    // Choix de l'adversaire
    std::cout << "Voulez-vous jouer contre l'ordinateur ? (oui/non) : ";
    std::cin >> choix;
    JouerContreOrdi = (choix == "oui");
    ...
    while (true) {
        ...
        if (checkVictory(grille, player)) {
            ...
            std::fill(&grille[0][0], &grille[0][0] + sizeof(grille) / sizeof(int), 0);
            drawGrid(grille);
            player = 1;
            continue;
        }
